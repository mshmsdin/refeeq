/**
 * Master Archiving Script for «وثائق منتدى محاورين» Telegram Documents
 * Imports 1,100+ documents into 8 Logical Thematic Doors under Shia sect
 */
...
const EXPORT_DIR = 'C:/Users/mshms/Downloads/Telegram Desktop/ChatExport_2026-08-22 (2)';
const TARGET_STORAGE = path.join(__dirname, '..', 'storage', 'shia', 'mohawereen');
...
const DOORS = {
  ...
  AHLULBAYT: '02 - باب أمهات المؤمنين وبنات النبي وآل البيت وفدك',
  SAHABA: '03 - باب الصحابة والخلفاء الراشدين والفتنة',
  IMAMATE: '04 - باب الإمامة والعصمة والغلو والولاية',
  MAHDI: '05 - باب الإمام المهدي والغيبة والسرداب ونرجس',
  FIQH: '06 - باب الفقه والعقائد الخاصة (المتعة، الخمس، الشعائر)',
  SCHOLARS: '07 - باب مراجع وعلماء وكتب الشيعة ونقد الروايات',
  HISTORY: '08 - باب التاريخ والسير والمناظرات العامة'
};

const ROOT_FOLDER = 'وثائق منتدى محاورين في الرد على الشيعة';
...
function classifyTopic(text, topic) {
  ...
  if (
    ...
    combined.includes('تحريف') || combined.includes('فصل الخطاب') || combined.includes('الناسخ') ||
    combined.includes('تفسير') || combined.includes('آيات') || combined.includes('آية')
  ) {
    return DOORS.QURAN;
  }

  // 2. Mahdi & Occultation & Narjis
  if (
    ...
    combined.includes('القائم') || combined.includes('السفراء') || combined.includes('المسردب') ||
    combined.includes('عقيم') || combined.includes('العسكري')
  ) {
    return DOORS.MAHDI;
  }

  // 3. Mothers of Believers & Ahlulbayt & Fadak
  if (
    combined.includes('عائشة') || combined.includes('حفصة') || combined.includes('أم المؤمنين') ||
    combined.includes('فاطمة') || combined.includes('الزهراء') || combined.includes('رقية') ||
    combined.includes('أم كلثوم') || combined.includes('بنات النبي') || combined.includes('فدك') ||
    combined.includes('الإفك') || combined.includes('كسر الضلع') || combined.includes('أهل البيت') ||
    combined.includes('اهل البيت') || combined.includes('الكساء') || combined.includes('زينب')
  ) {
    return DOORS.AHLULBAYT;
  }

  // 4. Companions & Caliphs & Fitnah
  if (
    combined.includes('أبو بكر') || combined.includes('ابو بكر') || combined.includes('عمر') ||
    combined.includes('عثمان') || combined.includes('الصحابة') || combined.includes('الشورى') ||
    combined.includes('بيعة') || combined.includes('الجمل') || combined.includes('صفين') ||
    combined.includes('معاوية') || combined.includes('عمرو بن العاص') || combined.includes('أبو هريرة') ||
    combined.includes('خالد بن الوليد') || combined.includes('بدر') || combined.includes('البدري') ||
    combined.includes('السقيفة') || combined.includes('رزية الخميس') || combined.includes('نهج البلاغة')
  ) {
    return DOORS.SAHABA;
  }
  ...
  if (
    ...
    combined.includes('الربوبية') || combined.includes('الألوهية') || combined.includes('معصوم') ||
    combined.includes('إمام زمانه') || combined.includes('وليد الكعبة')
  ) {
  ...
  if (
    ...
    combined.includes('بداء') || combined.includes('خمس') || combined.includes('لطم') ||
    combined.includes('تطبير') || combined.includes('شعائر') || combined.includes('عاشوراء') ||
    combined.includes('تراويح') || combined.includes('أذان') || combined.includes('تربة') ||
    combined.includes('سجود') || combined.includes('بول') || combined.includes('غائط')
  ) {
    return DOORS.FIQH;
  }
  ...
  if (
    ...
    combined.includes('الأنوار النعمانية') || combined.includes('البحراني') || combined.includes('السيستاني') ||
    combined.includes('الصدر') || combined.includes('الخميني') || combined.includes('رجال') ||
    combined.includes('سند') || combined.includes('معمم') || combined.includes('رويات')
  ) {
...
// PARSE HTML FILES
// ══════════════════════════════════════════════════════════
console.log('📖 قراءة وفهرسة ملفات تصدير: «وثائق منتدى محاورين»...');

const htmlFiles = ['messages.html', 'messages2.html'];
const parsedItems = [];
let currentTopic = 'وثائق منتدى محاورين';
let topicSeq = {};
...
for (const hf of htmlFiles) {
  ...
  while ((match = msgRegex.exec(content)) !== null) {
    ...
    if (text.length > 0 && text.length < 180 && !text.startsWith('http')) {
    ...
    if (photoHref || videoHref || text.length > 40) {
...
// ══════════════════════════════════════════════════════════
// PERSIST IN DATABASE & COPY MEDIA
// ══════════════════════════════════════════════════════════
const db = getDb();

console.log('🗑️ تهيئة وتحديث مجلدات وثائق منتدى محاورين...');
db.prepare("DELETE FROM ocr_boxes WHERE document_id IN (SELECT id FROM documents WHERE book_source LIKE '%منتدى محاورين%')").run();
db.prepare("DELETE FROM document_tags WHERE document_id IN (SELECT id FROM documents WHERE book_source LIKE '%منتدى محاورين%')").run();
db.prepare("DELETE FROM documents WHERE book_source LIKE '%منتدى محاورين%'").run();
db.prepare("DELETE FROM folders WHERE path LIKE 'وثائق منتدى محاورين%'").run();
...
const insertDoc = db.prepare(`
  ...
  ) VALUES (?, ?, ?, ?, ?, 'شيعة', 'attack', 'وثائق منتدى محاورين في الرد على الشيعة', ?, 'completed', ?)
`);
...
const runInsertBatch = db.transaction(() => {
  for (let idx = 0; idx < parsedItems.length; idx++) {
    ...
    const relPath = `mohawereen/${path.basename(destAbsMedia || 'text_doc')}`;
    ...
    insertTag.run(docId, 'منتدى محاورين', 1);
...
console.log('\n══════════════════════════════════════════════════════');
console.log(`🎉 تمت أرشفة «وثائق منتدى محاورين» بنجاح!`);
...
const doorStats = db.prepare(`
  ...
  FROM documents
  WHERE book_source LIKE '%منتدى محاورين%'
  GROUP BY folder_name
  ORDER BY folder_name
`).all();

console.log('\n📊 إحصائيات الأبواب الثمانية لمنتدى محاورين:');
